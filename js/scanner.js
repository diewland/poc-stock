...
navigator.mediaDevices.enumerateDevices().then(function(sourceInfos){
  var video_srcs = sourceInfos.filter(function(src){ return src.kind.indexOf('video') > -1; });
  console.log('video sources:', video_srcs);
  var hdConstraints = {
    audio: false,
    video: {
      optional: [{
        // XXX choose camera
        // sourceId: video_srcs[0].deviceId // first camera
        sourceId: video_srcs[video_srcs.length-1].deviceId // last camera should be back camera
      }]
    }
  };
